...
(function(window, angular, undefined) {
    ...
    jqLite.prototype.is = function(name) {
        ...
        if (angular.isString(name)) {
            if (name.substring(0, 1) == ':') {
                ...
                if (bool.test(name))
                    return angular.isDefined(el.attr((name.substring(1, name.length)).replace(/\ /g, '')))
                        ? true
                        : false;
                else if (/:visible|:hidden/.test(name)) {
                    var visible = angular.contains(el[0].ownerDocument, el) ||
                        (el.css('display') === 'none' || el.css('visibilty') === 'hidden')
                        ? false
                        : true;
    ...
    forEach({ scrollLeft: "pageXOffset", scrollTop: "pageYOffset" },
...
+function(window, angular, undefined) {
    ...
    angular.forEach(['Append', 'Prepend', 'Bind'],
        function(directive) {
            nqCoreApp.directive('nq' + directive,
            [
    ...
    [
        '$helpers', function($helpers) {
            ...
            color.toHex = function(h, s, b, a) {
                ...
                return '#' +
                    ((1 << 24) | (parseInt(rgb.r, 10) << 16) | (parseInt(rgb.g, 10) << 8) | parseInt(rgb.b, 10))
...
angular.module('ngQuantum.services.helpers', [])
    .factory('$helpers',
    [
        '$injector', '$window', function($injector, $window) {
            var fn = {};
                ...
                angular.forEach(array,
                    function(trigger) {
                        if (trigger === 'click') {
                            ...
                            trigger !== 'hover' &&
                                element.on(isTouch ? 'touchstart' : 'mousedown', $master.$onFocusElementMouseDown);
                        }
                    });
                return hasclick;
            }
            fn.unBindTriggers = function(element, triggers, $master) {
                ...
                for (var i = array.length; i--;) {
                    ...
                    if (trigger === 'click') {
                        ...
                        trigger !== 'hover' &&
                            element.off(isTouch ? 'touchstart' : 'mousedown', $master.$onFocusElementMouseDown);
                    }
                }
            }
            fn.stringParsers = [
...
angular.module('ngQuantum.services.mouse', [])
    ...
    .factory('$mouse',
    [
        '$injector', '$window', '$mouseConfig', function($injector, $window, $mouseConfig) {
                ...
                function($parse, $q) {
                    function ParseOptionsFactory(attr, $element) {
                        ...
                        $parseOptions.init = function() {
                            ...
                            displayFn = $parse(match[2] || match[1]), valueName = match[4] || match[6], keyName = match[
                                5], groupByFn = $parse(match[3] || ''), valueFn =
                                $parse(match[2] ? match[1] : valueName), valuesFn = $parse(match[7]);
                            ...
                            angular.forEach(element.children(),
                                function(value, key) {
                                    if (angular.element(value).is("option")) {
                                        ...
                                        angular.forEach(angular.element(value).children(),
                                            function(gval, gkey) {
    ...
    [
        '$helpers', function($helpers) {
            ...
            fn.ensurePosition = function($target, element, options) {
                var offset = options.insideFixed ? $target.position() : $target.offset(),
                    ww = window.screen.width,
                    dh = $helpers.docHeight(),
                    tw = $target.width(),
                    th = $target.height(),
                    eh = element.height(),
                    eo = options.insideFixed ? element.position() : element.offset(),
            ...
            function getPosition(element, options) {
                var el = element[0];
                var clipRect = (typeof el.getBoundingClientRect == 'function')
            ...
            function getPlacement(element, placement, position, container) {
                ...
                placement = placement == 'bottom' &&
                    position.top + position.height + actualHeight - docScroll > parentHeight
                    ? 'top'
                    : placement == 'top' && position.top - docScroll - actualHeight < 0
                    ? 'bottom'
                    : placement == 'right' && position.right + actualWidth > parentWidth
                    ? 'left'
                    : placement == 'left' && position.left - actualWidth < parentLeft ? 'right' : placement
                return placement;
            }
    ...
    angular.module('ngQuantum.alert', ['ngQuantum.popMaster', 'ngQuantum.services.helpers'])
        ...
        .provider('$alert',
            function() {
                ...
                this.$get = [
                    '$timeout', '$rootScope', '$popMaster', '$compile', '$helpers', '$sce', '$parse',
                        ...
                        function AlertFactory(config, title, alertType, placement) {
                            ...
                            function getContainer() {
                                ...
                                if (!container || container.length < 1) {
                                    container = angular
                                        .element('<div class="alert-container ' + options.placement + '"></div>');
                            ...
                            if (attr) {
                                ...
                                if (attr && angular.isDefined(options.directive)) {
                                    attr[options.directive] &&
                                        options.$scope.$watch(attr[options.directive],
                                            function(newValue, oldValue) {
    ...
    angular.module('ngQuantum.aside', ['ngQuantum.services.helpers'])
        .provider('$aside',
            function() {
                ...
                this.$get = [
                    '$rootScope', '$animate', '$timeout', '$window',
                    function($rootScope, $animate, $timeout, $window) {
                        ...
                        function Factory(element, config, attr) {
                            ...
                            var body = angular.element(options.offCanvasBody),
                                $container,
                                applyBody = options.pinnable || options.collapsible;
                            var classes = ['aside-pinned', 'aside-collapsed', 'aside-opened', 'aside-closed'], backDrop;


                            if (attr && attr.$$observers) {
                                angular.forEach(attr.$$observers,
                                    function(value, key) {
                            ...
                            function clearStyle() {
                                applyBody &&
                                    body.removeClasses([
                                        options.side + '-aside-opened', options.side + '-aside-collapsed',
                                        options.side + '-aside-pinned'
                                    ]);
        ...
        .directive("nqAside",
        [
            "$aside", '$helpers', function($aside, $helpers) {
                return {
                    restrict: "AC",
                    scope: true,
...
+function(window, angular, undefined) {
    ...
    angular.forEach(['CheckboxGroup', 'RadioGroup'],
        function(directive) {
            nqButtonsApp.directive('nq' + directive,
                function() {
                    return {
                        restrict: 'A',
                        require: 'ngModel',
                            ...
                            angular.forEach(children,
                                function(child) {
                                    ...
                                    if (dirType == 'checkbox') {
                                        ...
                                        (!childEl.attr('ng-true-value') && trueVal) &&
                                            childEl.attr('ng-true-value', trueVal);
                                        (!childEl.attr('ng-false-value') && falseVal) &&
                                            childEl.attr('ng-false-value', falseVal)
                                    } else
    ...
    angular.forEach(['Checkbox', 'Radio', 'Toggle'],
        function(directive) {
            nqButtonsApp.directive('nq' + directive + 'Button',
            [
                '$button', '$helpers', function($button, $helpers) {
                    return {
                        restrict: 'A',
                        require: 'ngModel',
                        link: function postLink(scope, element, attr, controller) {
                            ...
                            if (directive == 'checkbox') {
                                trueValue = angular.isDefined(attr.ngTrueValue)
                                    ? $helpers.parseConstant(attr.ngTrueValue)
                                    : true;
                                falseValue = angular.isDefined(attr.ngFalseValue)
                                    ? $helpers.parseConstant(attr.ngFalseValue)
                                    : false;
                            } else {
                                trueValue = attr.ngValue
                                    ? scope.$eval(attr.ngValue)
                                    : $helpers.parseConstant(attr.value);
                            }
                            ...
                            scope.$watch(attr.ngModel,
                                function(newValue, oldValue) {
                                    var isActive = angular
                                        .equals($helpers.parseConstant(controller.$modelValue), trueValue);
                            ...
                            if (!isInput) {
                                element.bind(options.toggleEvent,
                                    function() {
                                        var viewValue = directive == 'radio'
                                            ? trueValue
                                            : controller.$modelValue
                                            ? $helpers.parseConstant(controller.$modelValue) == trueValue
                                            ? falseValue
                                            : trueValue
                                            : trueValue;
    ...
    angular.module('ngQuantum.carousel', ['ngQuantum.services.helpers'])
        ...
        .provider('$carousel',
            function() {
                ...
                this.$get = [
                    '$timeout', '$interval', '$filter', '$compile', '$sce', '$animate',
                    function($timeout, $interval, $filter, $compile, $sce, $animate) {
                        function CarouselFactory($element, $scope, config) {
                            ...
                            var stopFunc;
                            angular.forEach(['next', 'prev', 'play', 'pause'],
                                function(value) {
                                    $scope['$' + value] = function(evt) {
                                        $carousel[value](evt);
                                    }
                                })
                            angular.forEach(['outerWidth', 'innerHeight'],
                                function(value) {
                            ...
                            $carousel.init = function() {
                                ...
                                $timeout(function() {
                                        if (parseInt($scope.$outerWidth) > $element.width()) {
                                            $scope
                                                .$innerHeight =
                                                ((parseInt($scope.$innerHeight) / parseInt($scope.$outerWidth)) *
                                                    $element.width()) +
                                                'px';
                                        }
                                    },
        ...
        .directive('nqCarousel',
        [
            '$carousel', '$helpers', function($carousel, $helpers) {
                return {
                    restrict: 'EA',
                    transclude: true,
                    replace: true,
                    ...
                    controller: [
                        '$scope', '$element', '$attrs', function($scope, $element, $attrs) {
                            ...
                            angular.forEach([
                                    'effect', 'speed', 'interval', 'keyboard', 'hoverStop', 'autoPlay', 'outerWidth',
                                    'innerHeight'
                                ],
                                function(key) {
                                    angular.isDefined($attrs[key]) &&
                                        (options[key] = $helpers.parseConstant($attrs[key]))
                                })
        ...
        [
            '$parse', function($parse) {
                return {
                    ...
                    replace: true,
                    template:
                        '<div class="item" carousel-item-transclude="" ng-swipe-left="$parent.$next()"  ng-swipe-right="$parent.$prev()"></div>',
                    transclude: true,
                    scope: {
                        active: '=?',
                        heading: '@',
                ...
                return {
                    ...
                    link: function(scope, elm, attrs, controller) {
                        ...
                        if (controller.$watchIndicator && scope.$first) {
                            ...
                            controller
                                .visibleThumb = parseInt((elm.closest('.carousel').width() - 100) /
                                    controller.thumbWidth)
                        }
                        scope.$watch(function() { return item.thumbImage },
                            function(image) {
                                if (image) {
                        ...
                        if (controller.$watchIndicator) {
                            scope.$watch(function() { return controller.$lastIndex },
                                function(index) {
        ...
        .directive('carouselItemTransclude',
        [
            '$animate', '$timeout', function($animate, $timeout) {
                return {
                    ...
                    link: function(scope, elm, attrs, controller) {
                        scope.$watch('$transcludeFn',
                            function(value) {
                                scope.$transcludeFn(scope.$parent,
                                    function(contents) {
                                ...
                                if ($collapse.collapsed) {
                                    ...
                                    setTimeout(function() {
                                            target.addClass('collapsing')[dimension](0)
                                                .transitionEnd(function() {
                                                    target[dimension]('');
                                                    target.removeClass('collapsing').removeClass('in')
                                                        .css(dimension, '');
                            ...
                            function(child, key) {
                                ...
                                if (target.length) {
                                    ...
                                    if (link.length) {
                                        ...
                                        link.attr('data-ng-click',
                                            model + "=" + model + "==" + key + "?" + 20000000 + ":" + key)
            ...
            '$collapse', function($collapse) {
                return {
                    ...
                    compile: function(tElm, tAttrs, transclude) {
                        var collapsed = true,
                            target = angular.element(tAttrs.targetId),
                            dimension = angular.isDefined(tAttrs.dimension) && tAttrs.dimension == 'width'
                                ? tAttrs.dimension
                                : 'height';
                        if (angular.isDefined(tAttrs.collapsed) &&
                            (tAttrs.collapsed == 'false' || tAttrs.collapsed == false))
                            collapsed = false;
                        var options = {
                            collapsed: collapsed,
                            dimension: dimension,
                        ...
                        return function postLink(scope, element, attr, controller) {
                            collapse &&
                                angular.isDefined(tAttrs.ngModel) &&
                                scope.$watch(attr.ngModel,
                                    function(value, old) {
                                        if (value == undefined)
                                            return;
    ...
    angular.module('ngQuantum.colorpicker', ['ngQuantum.popMaster'])
        ...
        .provider('$colorPicker',
            function() {
                ...
                this.$get = [
                    '$rootScope', '$popMaster', '$document', '$color', '$mouse', '$helpers',
                    function($rootScope, $popMaster, $document, $color, $mouse, $helpers) {
                        function Factory(element, config, attr) {
                            ...
                            $picker.init = function() {
                                init();
                                findElement();
                                scope.selectedColor = scope
                                    .newColor = scope.selectedColor || options.defaultColor || '#FF0000';
            ...
            '$colorPicker', function($colorPicker) {
                return {
                    ...
                    link: function postLink(scope, element, attr, controller) {
                        ...
                        picker.$scope.$watch('selectedColor',
                            function(newVal, oldVal) {
                ...
                this.$get = [
                    ...
                    function($compile, $popMaster, $parse, $helpers, $timeout) {
                        function Factory(element, config, attr, ngModel) {
                            ...
                            angular.forEach(['next', 'before', 'today', 'gotoYear'],
                                function(value) {
                                    scope['$' + value] = function(evt) {
                                        scope.$$postDigest(function() {
                                            $picker[value](evt);
                            ...
                            $picker.changeDate = function(dir, type, val) {
                                var v = !val
                                    ? 1
                                    : angular.isNumber(val) ? val : window.isNaN(parseInt(val)) ? 1 : parseInt(val);
                                v = dir == 'down' ? -v : v;
                                type = type || 'day';
                            ...
                            $picker.today = function(evt) {
                                ...
                                apply(function() {
                                    scope.currentDate = scope.minDate && scope.startDate < scope.minDate
                                        ? scope.minDate.clone()
                                        : scope.maxDate && scope.startDate > scope.maxDate
                                        ? scope.maxDate.clone()
                                        : scope.startDate.clone();
                                    buildNew()
                            ...
                            function createCache(cachekey) {
                                ...
                                for (var d = 0; d < diff; d++) {
                                    ...
                                    if (options.disableWeekdays.length &&
                                        options.disableWeekdays.indexOf(day.day()) > -1) {
                            ...
                            function detectDate(dDate, defaultDate) {
                                var value = angular.isString(dDate)
                                    ? moment(dDate, options.format)
                                    : angular.isDate(dDate)
                                    ? moment(dDate)
                                    : moment.isMoment(dDate) ? dDate.clone() : defaultDate;
                            ...
                            function optimize() {
                                ...
                                if (options.startDate) {
                                    ...
                                    scope.startDate = scope.minDate && moment() < scope.minDate
                                        ? scope.minDate.clone()
                                        : scope.maxDate && moment() > scope.maxDate ? scope.maxDate.clone() : moment();
                                ...
                                if (options.timepicker) {
                                    options.minHour = angular.isNumber(options.minHour) && (options.minHour >= 0)
                                        ? options.minHour
                                        : 8;
                                    options.maxHour = angular.isNumber(options.maxHour) && options.maxHour || 22;
                                    options.divideHour = angular
                                        .isNumber(options.divideHour) &&
                                        options.divideHour ||
                                        4;
                                    (options.minHour < 0 || options.minHour > 23) &&
                                        (options.minHour = 0)
                                        (options.maxHour > 24 || options.maxHour < 2) &&
                                        (options.maxHour = 24)
                            ...
                            function buildTable() {
                                ...
                                if (!table) {
                                    ...
                                    var names = options.dayHeader == 'shortest'
                                        ? moment.localeData()._weekdaysMin || moment.localeData()._weekdaysShort
                                    ...
                                    var trhead = angular.element('<tr><th>' + names.join('</th><th>') + '</th></tr>')
                                        .appendTo(thead);
                                    if (options.weekNumber) {
                            ...
                            function buildTableBody() {
                                ...
                                for (var r = 0; r < rows; r++) {
                                    ...
                                    angular.forEach(rdates,
                                        function(val, key) {
                                            var slday = val.month + '-' + val.day;
                                            var td = angular
                                                .element('<td cal-date-item="' +
                                                    idx +
                                                    '" ng-class="{selected:selectedDay==\'' +
                                                    slday +
                                                    '\'}">' +
                                                    val.day +
                                                    '</td>').appendTo(trdate);
                            ...
                            function buildYearSelector() {
                                if (options.showYears) {
                                    yearSelector = angular
                                        .element('<div class="year-selector" ng-hide="yearsArray.length < 2" nq-scroll="" data-qo-axis="x" data-qo-bar-size="slimmest" data-qo-placement-offset="0" data-qo-visible="true"></div>').appendTo($footer);
                                    var inner = angular.element('<div class="selector-inner"></div>')
                                        .appendTo(yearSelector);
                                    options.theme && yearSelector.attr('data-qo-theme', options.theme);
                                    getYearArray();
                                    inner
                                        .append('<a role="button" tabindex="1" id="year-{{year}}" ng-repeat="year in yearsArray" ng-click="$gotoYear(year, $event)" ng-class="{active:currentYear == year}"><span>{{year}}</span></a>');
                            ...
                            function buildTimeSelector() {
                                if (options.timepicker) {
                                    var dpCont = angular
                                            .element('<div role="presentation" tabindex="-1" class="dp-container clearfix"></div>').appendTo($body),
                                        tpCont = angular.element('<div class="tp-container clearfix"></div>')
                                            .appendTo($body),
                                        tpSwicher = angular
                                            .element('<div class="tp-switcher" time-picker-switch="" data-time-icon="$options.timeIcon"  data-close-icon="$options.closeIcon"></div>').appendTo(tpCont),
                            ...
                            function timePickerTemplate() {
                                ...
                                if (options.timeView != 'list') {
                                    ...
                                    tableTime += hh
                                        ? '<td><button ng-class="\'btn-\' + $options.theme" type="button" class="btn tp-up" ng-click="$changeTime(\'up\',\'hour\')"><i ng-class="$options.upIcon"></i></button></td>'
                                        : ''
                                    ...
                                    tableTime += ss
                                        ? '<td><button ng-class="\'btn-\' + $options.theme" type="button" class="btn tp-up" ng-click="$changeTime(\'up\',\'second\')"><i ng-class="$options.upIcon"></i></button></td>'
                                        : '';
                                    tableTime += '</tr><tr>';
                                    ...
                                    tableTime += '</tr><tr>';
                                    tableTime += hh
                                        ? '<td><button ng-class="\'btn-\' + $options.theme" type="button" class="btn tp-down" ng-click="$changeTime(\'down\',\'hour\')"><i ng-class="$options.downIcon"></i></button></td>'
                                        : '';
                                    tableTime += hh ? '<td class="tp-seperator"></td>' : '';
                                    tableTime += mm
                                    ...
                                    tableTime += mm ? '<td class="tp-seperator"></td>' : ''
                                    tableTime += ss
                                        ? '<td><button ng-class="\'btn-\' + $options.theme" type="button" class="btn tp-down" ng-click="$changeTime(\'down\',\'second\')"><i ng-class="$options.downIcon"></i></button></td>'
                                        : '';
                                    ...
                                    if (!options.timesSet.length) {
                                        ...
                                        angular.forEach(options.timesSet,
                                            function(val) {
                                                val = val.replace(' ', '')
                                    ...
                                    var timeListContainer = angular
                                        .element('<div class="tp-time-list" nq-scroll="" data-qo-bar-size="slimmest" data-qo-placement-offset="0" data-qo-visible="true"><a class="tp-time" role="button" tabindex="1" ng-repeat="time in timesList" tp-time-list-item="time"  ng-class="{active:$parent.currentTimeString == time}"></a></div>')
                            ...
                            function buildScope() {
                                angular.forEach(['onDateChange', 'onTimeChange', 'onChange'],
                                    function(key) {
                                                ...
                                                apply(function() {
                                                    ...
                                                    if (angular.isDate(newValue)) {
                                                        ...
                                                        dt =
                                                            ...
                                                            ? moment(newValue, scope.format)
                                                            : moment.isMoment(newValue) ? newValue : moment();

                                                    if (!dt.isValid())
                                                        throw 'Type Error: ' +
                                                            attr.ngModel +
                                                            ' is not a valid Date, moment or date string...';
                                                    if (options.timepicker) {
                                                        var hr = dt.hour()
                                                        if (hr < options.minHour)
                                                    ...
                                                    scope
                                                        .selectedDay =
                                                        scope.currentDate.month() + '-' + scope.currentDate.date();
                                ...
                                if (options.minDateFrom) {
                                    ...
                                    if (fromEl.length) {
                                        ...
                                        fromScope &&
                                            fromScope.$watch('modelDate',
                                                function(newValue, oldValue) {
                                                    if (newValue) {
                                                        apply(function() {
                                                            var dt = moment(newValue);
                                                            scope.minDate = dt.clone()
                                                                .add(options.minRange, options.rangeType);
                                                            scope.currentDate = dt.clone()
                                                                .add(options.defaultRange, options.rangeTypee);
                                                            scope
                                                                .selectedDay =
                                                            ...
                                                            options.minYear = scope.minDate.year();
                                                            if (options.maxRange) {
                                                                scope.maxDate = dt.clone()
                                                                    .add(options.maxRange, options.rangeType);
                                                                options.maxYear = scope.maxDate.year();
                                                            }
                                                            getYearArray()
                                                            buildNew();
                                        ...
                                        fromScope &&
                                            fromScope.$on(options.prefixEvent + '.hide',
                                                function() {
                                                    if (hasChage)
                                                        $picker.show();
                                                    hasChage = false;
                                ...
                                scope.$watch('currentDateObject.month',
                                    function(newval, oldval) {
                                        if (newval != oldval && oldval) {
                                            scope.selectedDay = scope.selectedDay.replace(oldval + '-', newval + '-');
                                        }
                                    })
                                scope.$on('pickerTimeChanged',
                                    function(evt, val) {
                                        formatPicker();
                                ...
                                scope.$on('pickerDatetimeChanged',
                                    function(evt, val) {
                                        formatPicker();
                                        if (angular.isFunction(options.onChange))
                                            options.onChange(scope, { $currentDate: scope.currentDate.toDate() });
                                    })
                                scope.$on('pickerDateChanged',
                                    function(evt, val) {
                                        formatPicker();
        ...
        .directive('timePickerSwitch',
            function() {
                return {
                    restrict: 'AC',
                    link: function postLink(scope, element, attr, transclusion) {
                        var span = angular.element('<a href="#" class="tps-btn"></a>')
                            .append('<i class="' + scope.$eval(attr.timeIcon) + '"></i>').appendTo(element)
                        ...
                        element.append('<a class="tp-close" title="Close"><i class="' +
                            scope.$eval(attr.closeIcon) +
                            '"></i></a>');
                        scope.$on('$destroy',
                            function() {
                                element.off('click')
            ...
            function() {
                return {
                    ...
                    link: function postLink(scope, element, attr, transclusion) {
                        ...
                        scope.$watch('currentDateObject.' + type,
                            function(newVal, oldVal) {
                                switch (type) {
                                case 'hour':
...
angular.module('ngQuantum.loading', ['ngQuantum.services.lazy'])
    .run([
        '$http', '$rootScope', '$timeout', function($http, $rootScope, $timeout) {
            $rootScope.$watch(function() { return $http.pendingRequests.length },
                function(newVal, oldVal) {
                    $rootScope.$pendingRequestCount = $rootScope.$pendingRequestCount || 0;
                    $timeout(function() {
    ...
    .provider('$loading',
        function() {
            ...
            this.$get = [
                '$timeout', '$rootScope', '$compile', '$http',
                function($timeout, $rootScope, $compile, $http) {
                    function LoadingFactory(config, theme, placement) {
                        ...
                        var container = angular.isElement(options.container)
                            ? options.container
                            : angular.element(options.container)
                        if (!container.length)
                        ...
                        $loading.updateProgress = function(rate) {
                            ...
                            if (rate)
                                ...
                                if (scope.currentRate < 100)
                                    scope.currentRate = scope.currentRate +
                                        (scope.currentRate < 80 ? 5 : (parseInt(100 - scope.currentRate) / 2));
                                $timeout(function() {
                        ...
                        $rootScope.$watch('$pendingRequestCount',
                            function(newVal, oldVal) {
                                if (newVal <= 0) {
...
angular.module('ngQuantum.loadingButton', ['ngQuantum.services.helpers'])
    ...
    .directive("nqLoadingButton",
    [
        '$parse', '$loadingButton', '$helpers', '$timeout', '$q',
        function($parse, $loadingButton, $helpers, $timeout, $q) {
            return {
                ...
                link: function(scope, element, attr) {
                    ...
                    angular.forEach(['onSuccess', 'onTimeout', 'onError'],
                        function(key) {
                            if (angular.isDefined(attr[key]))
                                options[key] = $parse(attr[key]);
                        })
                    angular.forEach(['loadingText', 'showIcons', 'timeout'],
                        function(key) {
                    ...
                    if (options.showIcons) {
                        successicon = angular.isElement(options.successIcon)
                            ? options.successIcon
                            : angular.element(options.successIcon)
                        erricon = angular.isElement(options.errorIcon)
                            ? options.errorIcon
                            : angular.element(options.errorIcon)
                    }
                    element.on('click',
                        function(event) {
                            scope.$apply(function() {
...
angular.module('ngQuantum.modal', ['ngQuantum.popMaster'])
    ...
    .provider('$modal',
        function() {
            ...
            this.$get = [
                '$window', '$compile', '$http', '$sce', '$timeout', '$helpers', '$popMaster',
...
angular.module('ngQuantum.modalBox', ['ngQuantum.modal'])
            ...
            this.$get = [
                ...
                function($modal, $parse, $helpers) {
                    function ModalBoxFactory(config, attr) {
                        ...
                        var options = angular.extend({}, defaults, config);
                        attr &&
                            angular.forEach(['afterOk', 'afterConfirm', 'afterCancel', 'afterCustom'],
                                function(key) {
                                    if (angular.isDefined(attr[key])) {
                                        options[key] = $parse(attr[key]);
                                    }
                        ...
                        angular.forEach(['okText', 'cancelText', 'confirmText', 'promptLabel'],
                            function(key) {
        ...
        function($modalBox, $helpers) {
            return {
                ...
                link: function postLink(scope, element, attr, transclusion) {
                    ...
                    angular.forEach([
                            'boxType', 'promptLabel', 'promptModel', 'alertTemplate', 'confirmTemplate',
                            'promptTemplate',
                            'showIcon', 'okText', 'cancelText', 'confirmText'
                        ],
                        function(key) {
                            if (angular.isDefined(attr[key]))
                                options[key] = $helpers.parseConstant(attr[key]);
                    ...
                    if (angular.isDefined(attr.contentTarget)) {
                        ...
                        if (!content.length) {
                            content = angular
                                .element('<span><span class="label label-warning">Warning :</span> No content element find</span>')
                        }
                        options.htmlObject = true;
                        options.buildOnShow = false;
    ...
    function(window, angular, undefined) {
            ...
            .provider('$popMaster',
                function() {
                    var defaults = this.defaults = {
                        ...
                        targetElement: false,
                        template: angular
                            .element('<div style="min-width:100px; padding:3px; border:1px; solid #aaa">You should prepare a content will replace here...</div>'),
                        contentTemplate: false,
                        trigger: 'hover focus',
                        keyboard: false,
                        html: false,
                    ...
                    this.$get = [
                        '$window', '$rootScope', '$compile', '$parse', '$http', '$timeout', '$animate', '$placement',
                        '$helpers', 'templateHelper', '$q',
                        function($window,
                            $rootScope,
                            $compile,
                            $parse,
                            ...
                            function MasterFactory(element, config) {
                                ...
                                angular.forEach(['hide', 'show', 'toggle'],
                                    function(value) {
                                    ...
                                    if (!options.buildOnShow || options.show) {
                                        if (options.targetElement) {
                                            ...
                                            if (angular.isElement(options.template))
                                                ...
                                                $master.$promise = templateHelper
                                                    .fetchContent(angular.element(options.template));
                                            shouldCompile = true;
                                        }
                                        ...
                                        $master.$promise.then(function(template) {
                                            if (angular.isElement(template))
                                                ...
                                                if (options.htmlObject)
                                                    template = template
                                                        .replace(/ng-bind="content"/gi, 'nq-bind="content"')
                                ...
                                $master.show = function(callback) {
                                    ...
                                    if (options.effect) {
                                        if (options.displayReflow) {
                                            ...
                                            $animateTarget.removeClass('ng-animate').removeClass(options.speed)
                                                .removeClass(options.effect + '-remove')
                                                .removeClass(options.effect + '-remove-active');
                                            ...
                                            promise = $animate.addClass($animateTarget, options.effect)
                                                .then(function() {
                                                    complateShow(callback);
                                                    $master.fireEvents('show');
                                                    $animateTarget.show();
                                ...
                                $master.hide = function(callback) {
                                    ...
                                    if (options.effect) {
                                        if (options.displayReflow)
                                            ...
                                            promise = $animate.removeClass($animateTarget, options.effect)
                                                .then(function() {
                                                    $animateTarget.hide();
                                                    if ($target) {
                                                        $target.hide();
                                ...
                                $master.$applyPlacement = function() {
                                    ...
                                    if (!options.preventReplace) {
                                        $placement
                                            .applyPlacement($master.$toElement && $master.$toElement ||
                                                $master.$currentElement && $master.$currentElement ||
                                                element,
                                                $target,
                                                options)
                                ...
                                $master.applyEvents = function(status) {
                                    ...
                                    if (angular.isDefined(options.onShow))
                                        scope.$on(options.prefixEvent + '.show',
                                            function() {
                                                options.$scope.$eval(options.onShow);
                                                options.$scope.$apply();
                                            });
                                    if (options.onHide)
                                        scope.$on(options.prefixEvent + '.hide',
                                            function() {
                                ...
                                function onBodyClick(evt) {
                                    ...
                                    if (evt.target === elm[0])
                                        ...
                                        return false;
                                    else if ((options.multiple || options.overseeingTarget) &&
                                        (evt.target == $master.$target[0] || $master.$target.has(evt.target)))
                                        return false;
                                    return evt.target !== elm[0] && $master.leave();
                                }

                                function outerHoverTrigger(evt) {
                                    if ($master.$target[0] == evt.target ||
                                        $master.$target.has(angular.element(evt.target))) {
                                ...
                                function build() {
                                    if (options.container === 'self') {
                                        ...
                                        if (modal && modal.length)
                                            ...
                                            $container = angular.isElement(options.container)
                                                ? options.container
                                                : angular.element(options.container);
                                    }
                                    if (!element && (!$container || $container.length < 1))
                                        $container = angular.element('body');
                                    $target = $master.$target = shouldCompile
                                        ? $compile(linker)(scope,
                                ...
                                function complateHide(callback) {
                                    ...
                                    $target &&
                                        $target.css({ top: '', left: '' }).removeClass(lastplacement)
                                        .removeClass(options.speed);
                                    if (options.theme)
                                        $target.removeClass(options.theme)
                                            .removeClass(options.instanceName + '-' + options.theme);
                                ...
                                function ensureFixedPlacement() {
                                    if (options.ensurePlacement &&
                                    (!options
                                        .container ||
                                        ($container && $container.css("position") === "fixed"))) {
                                        ...
                                        angular.forEach($checkElements,
                                            function(node) {
                                                ...
                                                if (fx.css("position") === "fixed") {
                                                    ...
                                                    if ((val != 'auto' || val != 'initial') &&
                                                        parseInt(val) < scaleH &&
                                                        options.placement.indexOf("top-") > -1) {
                                                    ...
                                                    if ((val != 'auto' || val != 'initial') &&
                                                        parseInt(val) < scaleW &&
                                                        options.placement.indexOf("left-") > -1) {
                                                    ...
                                                    if ((val != 'auto' || val != 'initial') &&
                                                        parseInt(val) < scaleW &&
                                                        options.placement.indexOf("right-") > -1) {
        ...
        angular.forEach(['Modal', 'Dropdown', 'Popover'],
            function(directive) {
                app.directive('nqToggle' + directive,
                [
                    '$helpers', '$rootScope', '$sce', '$timeout', function($helpers, $rootScope, $sce, $timeout) {
                        return {
                            restrict: 'EA',
                            priority: 1000,
                            link: function postLink(scope, element, attr, transclusion) {

                                var itemId = attr['nqToggle' + directive
                                    ],
                                    target,
                                    service,
                                    options = {},
                                ...
                                function build() {
                                    ...
                                    trigger = angular.isDefined(attr.trigger)
                                        ? attr.trigger
                ...
                function($popMaster, $sce, $helpers) {
                    function PopoverFactory(element, config, attr) {
                        ...
                        if (attr) {
                            ...
                            if (attr && angular.isDefined(options.directive)) {
                                attr[options.directive] &&
                                    scope.$watch(attr[options.directive],
                                        function(newValue, oldValue) {
                                            if (angular.isObject(newValue)) {
                                                angular.extend(scope, newValue);
                                            } else {
        ...
        function($popover) {
            return {
                ...
                link: function postLink(scope, element, attr) {
                    ...
                    if (angular.isDefined(attr.qsTitle) ||
                        angular.isDefined(attr.qsContent) ||
                        attr.nqPopover ||
                        angular.isDefined(attr.qoTemplate) ||
                        angular.isDefined(attr.qoContentTemplate))
...
angular.module('ngQuantum.scrollbar', ['ngQuantum.services.helpers', 'ngQuantum.services.mouse'])
    .provider('$scrollbar',
        function() {
            ...
            this.$get = [
                ...
                '$compile', '$timeout',
                '$q', '$rootScope', '$helpers',
                function($mouse, $window, $compile, $timeout, $q, $rootScope, $helpers) {
                    ...
                    function Factory(element, config, attr) {
                        ...
                        angular.forEach(['increase', 'decrease'],
                            function(value) {
                        ...
                        function buildTemplate() {
                            ...
                            if (/y|both/.test(options.axis)) {
                                $y.bar = angular.element('<div class="scrollbar vertical-bar"></div>').appendTo(barBody)
                                    .addClass('bar-' + options.verticalPlacement);
                                $y.track = angular.element('<div class="scrollbar-track"></div>').appendTo($y.bar);
                                $y.trackInner = angular.element('<div class="track-inner"></div>').appendTo($y.track)
                                    .css('width', $size.barSize);
                                $y.thumb = angular.element('<div class="scrollbar-thumb"></div>')
                                    .appendTo($y.trackInner);
                                if (options.showButtons) {
                                    $y.increment = angular
                                        .element('<a class="bar-increment" ng-click="$increase(\'y\')"><span></span></a>').appendTo($y.bar).css('height', $size.buttonSize)
                                    $y.decrement = angular
                                        .element('<a class="bar-decrement" ng-click="$decrease(\'y\')"><span></span></a>').prependTo($y.bar).css('height', $size.buttonSize)
                                    $compile($y.bar)(scope);
                                }
                            ...
                            if (/x|both/.test(options.axis)) {
                                ...
                                $x.bar = angular.element('<div class="scrollbar horizontal-bar"></div>')
                                    .appendTo(barBody).addClass('bar-' + options.horizontalPlacement);
                                $x.track = angular.element('<div class="scrollbar-track"></div>').appendTo($x.bar);
                                $x.trackInner = angular.element('<div class="track-inner"></div>').appendTo($x.track)
                                    .css('height', $size.barSize);
                                $x.thumb = angular.element('<div class="scrollbar-thumb"></div>')
                                    .appendTo($x.trackInner);
                                if (options.showButtons) {
                                    $x.increment = angular
                                        .element('<a class="bar-increment" ng-click="$increase(\'x\')"><span></span></a>').appendTo($x.bar)
                                    $x.decrement = angular
                                        .element('<a class="bar-decrement" ng-click="$decrease(\'x\')"><span></span></a>').prependTo($x.bar)
                                    $compile($x.bar)(scope)
                            ...
                            !isTouch &&
                                angular.element('body').on('mouseup',
                                    function(evt) {
                                        angular.element(document).off('.scrollbar')
                                    });
                        ...
                        function buildEvents() {
                            if (!scope.useWebkit || isTouch) {
                                if (options.allDigest)
                                    ...
                                    if (/y|both/.test(options.axis))
                                        scope.$watch(function() { return el[0].scrollHeight },
                                            function(newVal, oldVal) {
                                                setTimeout(function() {
                                                        watchResult();
                                                    },
                                    ...
                                    if (/x|both/.test(options.axis))
                                        scope.$watch(function() { return el[0].scrollWidth },
                                            function(newVal, oldVal) {
                                                setTimeout(function() {
                                                        watchResult();
                                                    },
                                ...
                                if (!isTouch) {
                                        ...
                                        function(e) {
                                            ...
                                            if (options.keyboard) {
                                                ...
                                                angular.element(document)
                                                    .on('keydown.scrollbarkeyboard', $bar.$onKeyDown);
                                            }
                                    ...
                                    element.on('touchstart',
                                        function(event) {
                                            ...
                                            var sTouch = event.originalEvent.touches[0] ||
                                                event.originalEvent.changedTouches[0];
                                            $y.bar && $y.bar.css('visibility', 'visible');
                                            $x.bar && $x.bar.css('visibility', 'visible');
                                            angular.element(document).on('touchmove.scrollbar',
                                                function(evt) {
                                                    var touch =
                                                    ...
                                                    if (/y|both/.test(options.axis)) {
                                                        if (!$y.maxOffset && options.axis == 'y')
                                                            retuned = true;
                                                        if (
                                                            scope.scrollTop == $y.maxOffset &&
                                                                sTouch.pageY > touch.pageY)
                                                            retuned = true;
                                                    ...
                                                    if (/x|both/.test(options.axis)) {
                                                        if (!$x.maxOffset && options.axis == 'x')
                                                            retuned = true;
                                                        if (
                                                            scope.scrollLeft == $x.maxOffset &&
                                                                sTouch.pageX > touch.pageX)
                                                            retuned = true;
                                    ...
                                    angular.element(document).on('touchend.scrollbar touchcancel.scrollbar',
                                        function(evt) {
                        ...
                        function watchResult() {
                            ...
                            if (/y|both/.test(options.axis)) {
                                if (height > 0 &&
                                    ...
                                    $container &&
                                    $container.outerHeight() >= scope.maxHeight &&
                            ...
                            if (/x|both/.test(options.axis)) {
                                if (width > 0 &&
                                    scope.maxWidth < width &&
                                    $container.outerWidth() >= scope.maxWidth &&
                                    $container &&
                        ...
                        function checkAdaptable() {
                            ...
                            if (/ul|ol/.test(tag.toLowerCase()) && !scope.useWebkit) {
                                $barListItem = angular.element('<li class="scrollbar-list-item"></li>')
                                    .appendTo(element);
                                $container = element;
                            } else if (tag.toLowerCase() == 'table') {
                                $container = angular.element('<div class="scrollbar-container"></div>')
                        ...
                        function findSizes() {
                            ...
                            scope.maxWidth = angular.isNumber($size.width)
                                ? $size.width
                                : angular.isString($size.width)
                                ? $size.width.indexOf('%') !== -1
                                ? (element.parent().innerWidth() * ((parseFloat($size.width) / 100) || 0))
                                : parseFloat($size.width) || 0
                                : 0;
                            scope.maxHeight = angular.isNumber($size.height)
                                ? $size.height
                                : angular.isString($size.height)
                                ? $size.height.indexOf('%') !== -1
                                ? function() {
                                    exchild = true;
                                    return element.parent().innerHeight() * ((parseFloat($size.height) / 100) || 0)
                                }()
                                : parseFloat($size.height) || 0
                                : 0;
                        }
    ...
    function(window, angular, undefined) {
                    ...
                    this.$get = [
                        ...
                        function($filter,
                            ...
                            function SelectFactory(element, controller, config, attr, targetEl) {
                                ...
                                var searchInput = angular
                                    .element([
                                        '<input ng-hide="$hideFilter" ng-model="filterModel.label" type="text" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" placeholder="{{$placeholder}}" class="select-input form-control" role="combobox" aria-expanded="true"',, ' aria-autocomplete="list" style="max-width:100%;" />'
                                    ].join(""))
                                ...
                                if (options.filterable) {
                                    if (angular.isString(options.noMatch) &&
                                        options.noMatch.length > 2 &&
                                        options.noMatch.substr(0, 1) == '#')
                                ...
                                scope.$placeholder = options.displayType == 'input'
                                    ? options.placeholder
                                    : options.filterText;
                                ...
                                $select.init = function() {
                                    ...
                                    if (options.filterable && $target) {
                                        ...
                                        if (options.displayType == 'input') {
                                            ...
                                            inputItem = angular.element('<li></li>')
                                                .append(searchInput.removeClass('form-control').removeClass('input-xs'))
                                    ...
                                    if (options.remoteSearch) {
                                        charLabel = angular.element('<span ng-show="$remainingChar > 0"></span>')
                                            .append(options.charText)
                                        searchLabel = angular
                                            .element('<span class="search-label" ng-show="$dataLoading"></span>')
                                            .append(options.searchingText)
                                ...
                                $select.updateMatches = function(matches, isfilter) {
                                    ...
                                    if (isTagsInput, isfilter) {
                                        scope
                                            .$noResultFound = !((
                                                $filter('filter')(scope.$matches,
                                                    function(val) { return val.filtered != true })).length)
                                ...
                                $select.select = function(item) {
                                    if (options.multiple) {
                                        if (!item.selected &&
                                            options.maxLength &&
                                            (controller.$modelValue &&
                                                controller.$modelValue.length == options.maxLength))
                                            return
                                        else {
                                            scope.$apply(function() {
                                            ...
                                            var selected = []
                                            $filter('filter')($select.optionData,
                                                function(opt) {
                                ...
                                $select.$onKeyDown = function(e) {
                                    ...
                                    if (index == -1) {
                                        ...
                                        index = angular.isDefined(sIndex)
                                            ? sIndex
                                            : elSelected.hasClass('select-option')
                                            ? elSelected.parent().index()
                                            : elSelected.closest('.select-option').index();
                                    }
                                    ...
                                    if (e.keyCode === 13) {
                                        var match =
                                            $filter('filter')(scope.$matches,
                                                function(itm) { return itm.filtered != true })[index]
                                        if (match)
                                ...
                                $select.show = function() {
                                    ...
                                    promise &&
                                        promise.then(function() {
                                            ...
                                            $select.$target
                                                .on(isTouch ? 'touchstart' : 'mousedown', $select.$onMouseDown);
                                        })
                                };
                                    ...
                                    if (controller.$modelValue && options.modelIsLabel) {
                                        if ($select.complated) {
                                            if (angular.isArray(controller.$modelValue)) {
                                                angular.forEach(controller.$modelValue,
                                                    function(val) {
                                                        var
                                                            item =
                                                            {
                                ...
                                if (angular.isDefined(attr.ngChange)) {
                                    scope.$parent.$watch(function() { return controller.$modelValue },
                                        function(newValue, oldValue) {
                                ...
                                function renderSelected() {
                                    ...
                                    if (angular.isArray(controller.$modelValue)) {
                                        ...
                                        if (index > -1) {
                                            $select.optionData[index].selected = true
                                            inputItem
                                                .before(renderItem($select.optionData[index], controller.$modelValue));
                                        }
                                    }
                                    $select.$isShown &&
                                ...
                                function localFiler(newValue, oldValue) {
                                    if (!options.customFilter) {
                                        if (newValue) {
                                            $select
                                                .updateMatches($filter('filter')($select.optionData, scope.filterModel),
                                                    isTagsInput);
                                ...
                                function buildUrl(term, isPost) {
                                    ...
                                    if (term) {
                                        if (scope.urlParams.length) {
                                            angular.forEach(scope.urlParams,
                                ...
                                function validateModel() {
                                    ...
                                    if (options.minRequired) {
                                        if (angular.isArray(controller.$modelValue))
                                            controller
                                                .$setValidity("min-required",
                                                    controller.$modelValue.length >= options.minRequired);
                                    }
                                }

                                function clearSelected() {
                                    angular.forEach($select.optionData,
                                        function(item) {
                                            item.selected = false;
                                        })
                                }
                    ...
                    this.$get = [
                        ...
                        function($select, $filter) {
                            function TagsInputFactory(element, controller, config, attr, targetEl) {
                                ...
                                attr.nqTagsInput &&
                                    scope.$parent.$watch(attr.nqTagsInput,
                                        function(newValue, oldValue) {
                                            if (angular.isArray(newValue)) {
        ...
        angular.forEach(['nqSelect', 'nqTagsInput'],
            function(directive) {
                        ...
                        return {
                            ...
                            link: function postLink(scope, element, attr, controllers) {
                                ...
                                if (element.is('select') || element.is('input')) {
                                    ...
                                    var scroller = angular
                                        .element('<div tabindex="-1" class="scrollable" role="listbox"></div>');

                                    scroller.append(targetEl.show());
                                    options.targetElement = angular
                                        .element('<div tabindex="-1" role="listbox" class="listbox-panel"></div>')
                                        .append(scroller);
                                ...
                                function buildElement() {
                                    if (options.displayType == 'input' || attr.qoDisplayType == 'input') {
                                        ...
                                        element = angular
                                            .element('<ul class="nav nav-pills select-render-nav form-control"></ul>');
                                        targetEl.hide()
                                    } else
                                        element = angular
                                            .element('<button type="button" class="btn form-control">Please select...</button>');
            ...
            function() {
                return {
                    ...
                    link: function postLink(scope, element, attr, controller) {
                        ...
                        else if (angular.isDefined(element.parent().attr('ng-repeat') ||
                            element.parent().attr('data-ng-repeat'))) {
                            var parentattr = element.parent().attr('ng-repeat') ||
                                element.parent().attr('data-ng-repeat')
                            if (parentattr)
                        ...
                        if (controller) {
                            scope.$watch(controller.changeOption,
                                function(newValue, oldValue) {
                            ...
                            scope.$watch('_selectOption.filtered',
                                function(newValue, oldValue) {
            ...
            '$filter', function($filter) {
                return function(array) {
                    ...
                    function addToGroup(value) {
                        var group = $filter('filter')(newArray,
                            function(gr) {
...
angular.module('ngQuantum.slider', ['ngQuantum.services.mouse', 'ngQuantum.services.helpers'])
    .provider('$slider',
        function() {
            ...
            this.$get = [
                '$rootScope', '$document', '$mouse', '$parse',
                function($rootScope, $document, $mouse, $parse) {
                    function Factory(element, config) {
                        ...
                        $slider.init = function() {
                            getTemplate();
                            template
                                .addClass('slider-' + (options.direction == 'vertical' ? 'vertical' : 'horizontal'));
                            if (options.size) {
                                options.direction == 'vertical'
                                    ? template.height(options.size)
                                    : template.width(options.size)
                            }
                            options.sizeClass && template.addClass('slider-' + options.sizeClass)
                            findSizes();
                            bindMouse();
                            var titipclass = 'titip-' +
                                (options.direction == 'vertical' ? 'left' : 'top'
                                ) +
                                ' titip-sm';
                            ...
                            if (options.doubleThumb)
                                options.direction == 'vertical'
                                    ? ($slider.lastOffset2 = sizes.trh)
                                    : ($slider.lastOffset2 = sizes.trw);
                            $slider.setValues();
                        ...
                        function slideVertical(event) {
                            ...
                            (options.doubleThumb &&
                                $slider.eventNo == 0 &&
                                ((x > $slider.lastOffset2) ||
                                    (Math.abs(x - $slider.lastOffset) > Math.abs(x - $slider.lastOffset2))))) {
                        ...
                        function getValue(val) {
                            ...
                            return parseFloat(Math
                                .round(((val * sizes.stepRate) + parseFloat(options.min)) * 100) /
                                100).toFixed(options.decimalPlace)
                        }
                        ...
                        function findStep() {
                            ...
                            options.diff &&
                                options.doubleThumb &&
                                (sizes.diffPixel = Math.round(sizes.stepSize * options.diff));

                        }

                        function getTemplate() {
                            template = angular.element('<div class="range-slider"></div>').insertBefore(element)
                                .append(element)
                        ...
                        function buildRuller() {
                            ...
                            if (options.showLabel)
                                ruller.append('<div class="slider-values"><div class="values-min">' +
                                    getValueFormat(options.min) +
                                    '</div><div class="values-max">' +
                                    getValueFormat(options.max) +
                                    '</div></div>');
    ...
    .directive('nqSlider',
    [
        '$slider', '$helpers', function($slider, $helpers) {
            return {
                restrict: 'AC',
                require: 'ngModel',
                link: function postLink(scope, element, attr, controller) {
                    ...
                    var keys = [
                        'disabled', 'keyboard', 'decimalPlace', 'step', 'min', 'max', 'doubleThumb', 'showLabel',
                        'diff', 'size', 'sizeClass', 'showRuller', 'direction', 'theme', 'thumbClass', 'valuePrefix',
                        'valueSuffix', 'formatValue'
                    ]
...
angular.module('ngQuantum.switchButton', ['ngQuantum.services.helpers'])
    ...
    .directive('nqSwitchButton',
    [
        '$switchButton', '$helpers', function($switchButton, $helpers) {
            return {
                restrict: 'AC',
                scope: {},
                ...
                link: function postLink(scope, element, attr, controller) {
                    ...
                    angular.forEach(['theme', 'effect', 'btnSize'],
                        function(key) {
                            if (angular.isDefined(attr[key])) {
                                options[key] = attr[key];
                            }
                        });
                    angular.forEach(['trueLabel', 'falseLabel', 'labelText'],
                        function(key) {
                            if (angular.isDefined(attr[key])) {
                                options[key] = $helpers.parseConstant(attr[key])
                            }
...
angular.module('ngQuantum.tabset', ['ngQuantum.services.helpers'])
    ...
    .provider('$tabset',
        function() {
            ...
            this.$get = [
                '$timeout', '$filter', '$compile', '$sce', '$animate',
    ...
    .directive('nqTabset',
    [
        '$tabset', '$compile', '$timeout', '$helpers', function($tabset, $compile, $timeout, $helpers) {
            return {
                restrict: 'EA',
                transclude: true,
                replace: true,
                ...
                controller: [
                    '$scope', '$element', '$attrs', function($scope, $element, $attrs) {
                        ...
                        var options = {};
                        angular.forEach([
                                'theme', 'justified', 'effect', 'type', 'speed', 'placement', 'keyboard', 'trigger',
                                'responsive'
                            ],
                            function(key) {
                ...
                link: function postLink(scope, elm, attrs, controller, transcludeFn) {
                    attrs.nqTabset &&
                        scope.$parent.$watch(attrs.nqTabset,
                            function(newValue, oldValue) {
                                var content = elm.find('.tab-content');
                                if (newValue && newValue.length && content.length) {
                                    for (var j = 0; j < newValue.length; j++) {
                                        var paneelm = angular.element('<div data-nq-tab=""></div>')
                                            .append('<span data-tab-heading="">' + newValue[j].heading + '</span>')
                    ...
                    if (controller.$options.responsive) {
                        scope.$watch(function() { return elm.width(); },
                            function(newValue, oldValue) {
                                setTimeout(function() {
                                        scope.$apply(function() {
                    ...
                    function responsiveDesign(value, oldValue) {
                        ...
                        if (value < controller.panesWidth) {
                            if (controller.panes.length <= controller.lastStoredIndex ||
                                controller.lastStoredIndex == -1)
            ...
            return {
                ...
                compile: function(elm, attrs, transclude) {
                    return function postLink(scope, elm, attrs, controller) {
                        ...
                        if (attrs.disabled) {
                            scope.$parent.$watch($parse(attrs.disabled),
                                function(value) {
        ...
        '$compile', function($compile) {
            return {
                ...
                link: function(scope, elm, attrs, controller) {
                    ...
                    scope.$watch(function() { return pane.headingElement },
                        function(heading) {
                            if (heading) {
                ...
                function($sce, $rootScope, $popMaster, $helpers) {
                    ...
                    function TooltipFactory(element, config, attr) {
                        ...
                        if (attr) {
                            ...
                            attr.$$observers &&
                                attr.$$observers.qsTitle &&
                                attr.$observe('qsTitle',
                                    function(newValue, oldValue) {
...
(function(window, angular) {
    ...
    angular.forEach(['pageableRepeat', 'pageableRepeatStart', 'pageableRepeatEnd'],
        function(directive, dIndex) {
            pApp.directive(directive,
            [
                '$compile', function($compile) {
                    return {
                        require: '^nqPageable',
                        ...
                        compile: function(tElm, tAttrs, transclude) {
                            ...
                            if (dIndex < 2) {
                                ...
                                var arrkey = repeatString.substr(repeatString.indexOf("in"), repeatString.length)
                                    .split(' ')[1].split('=')[0];
            ...
            return {
                ...
                link: function postLink(scope, element, attr, controller) {
                    ...
                    if (copt.selectionMode == 'row' && copt.selectable) {
                        ...
                        scope.$watch(controller.rowField,
                            function() {
                                element.removeClass(copt.selectionClass);
                            });
                        scope.$on('$refreshPager',
                            function() {
                                element.removeClass(copt.selectionClass);
                            })
                    }
...
(function(window, angular) {
        ...
        editApp = angular.module('ngQuantum.pageable.editing', ['ngQuantum.services.templateHelper'])
            .provider('$pageableTemplate',
                function() {
                    ...
                    this.$get = [
                        'templateHelper', '$timeout', '$animate', '$compile',
                        function(templateHelper, $timeout, $animate, $compile) {
                            function Factory(element, config, controller) {
                                ...
                                var initialize = function(scope, tType) {
                                    ...
                                    function _show() {
                                        ...
                                        if (container == 'over') {
                                            ...
                                            if (tType == 'insert')
                                                ...
                                                after = scope.$isPageableItem && scope.$index > -1
                                                    ? items.eq(scope.$index)
                                                    : false;
                                        } else {
                                            parent = panel;
                                            after = container == 'self' || options.enterMethod == 'append'
                                                ? false
                                                : panel.children().last();
                                        }
                                        $timeout(function() {
                                                viewEl.addClass('pageable-template-wrapper')
                                                    .addClass(options.effectClasses)
                                    ...
                                    function getRowField() {
                                        $timeout(function() {
                                            var extend = tType == 'insert'
                                                ? scope.$eval(options.insertDefault)
                                                : scope[controller.rowField];
                                            $child[options.alias] = {};
                                    ...
                                    $child.$back = function() {
                                        ...
                                        viewEl &&
                                            viewEl.length &&
                                            $animate.leave(viewEl).then(function() { viewEl.remove() });
                                    ...
                                    if (tType !== 'detail') {
                                        function setRowField() {
                                            $timeout(function() {
                                                if (tType !== 'insert')
                                                    scope[controller.rowField] = angular
                                                        .copy($child[options.alias], {});
                                                options.onChange && scope.$eval(options.onChange);
                                            })
                                        ...
                                        function save() {
                                            ...
                                            if (options.disableRemote) {
                                                tType == 'edit'
                                                    ? setRowField()
                                                    : controller.addRow($child[options.alias]);
                                                $child.$back();
                                            } else {
                                                var promise = controller
                                                    .saveRow($child[options.alias], tType == 'insert');
                                                promise.then(function() {
                                                    $child.$back();
                                                })
                                            }
            ...
            .directive('nqPageableTemplate',
            [
                '$pageableTemplate', function($pageableTemplate) {
                    return {
                        restrict: 'A',
                        require: '^nqPageable',
                        terminal: true,
    ...
    angular.forEach(['Edit', 'Insert', 'Detail'],
        function(directive, index) {
            editApp.directive('nqShow' + directive,
                function() {
                    return {
                        restrict: 'A',
                        require: '^nqPageable',
...
angular.module('ngQuantum.pageable.factory', ['ngQuantum.services.helpers'])
    .provider('$pageable',
        function() {
            ...
            this.$get = [
                '$timeout', '$filter', '$http', '$rootScope', '$parse', '$helpers', '$q', '$lazyRequest',
                    ...
                    function Factory(element, config, attr) {
                        ...
                        if (attr) {
                            if (angular.isDefined(attr.qoAllOptions)) {
                                ...
                                angular.forEach([
                                        'formatRequest', 'formatData', 'loadError', 'deleted', 'updated', 'inserted',
                                        'onRefresh'
                                    ],
                                    function(val) {
                        ...
                        $pageable.loadRemote = function() {
                            ...
                            if (options.formatRequest) {
                                ...
                                if (post) {
                                    ...
                                    prm = angular.extend({},
                                        prm,
                                        obj.params.pageParams,
                                        obj.params.sortParams,
                                        { 'searchTerm': obj.params.searchTerm });
                                    ajax.url = buildUrl(prm);
                                }
                        ...
                        $pageable.sortTable = function(field, direction) {
                            ...
                            if (options.remotePaging) {
                                ...
                                if (multiple) {
                                    ...
                                    modelData =
                                        $filter('orderBy')(sortedData, field, direction == 'desc' ? true : false);
                                } else
                                    modelData =
                                        $filter('orderBy')($pageable.localdata,
                                            field,
                        ...
                        $pageable.searchTable = function(fields, term) {
                            ...
                            if (options.remotePaging) {
                                ...
                                modelData = $filter('filter')($pageable.localdata,
                                    function(item, key) {
                                        var match = false, i = 0;
                        ...
                        function callFormatUrl(obj) {
                            if (options.formatRequest) {
                                var called = options
                                    .formatRequest(scope,
                                        { $url: obj.url, $params: obj.params, $eventType: obj.eventType });
                        ...
                        function splicePages(pageNumbers, pages) {
                            ...
                            if (scope.$currentPage < pageNumbers || scope.$totalPages == pageNumbers)
                                ...
                                if (pages.length < pageNumbers || scope.$totalPages == scope.$currentPage) {
                                    var start = scope.$totalPages - pageNumbers > 0
                                        ? scope.$totalPages - pageNumbers
                                        : 0;
                                    newPages = allPages.slice(start, spliceValue(scope.$totalPages))
                                } else if (scope.$currentPage == pages[pages.length - 1])
                                    newPages = allPages
                                        .slice(scope.$currentPage - 1,
                                            spliceValue(pageNumbers + scope.$currentPage - 1));
                                else if (scope.$currentPage == pages[0])
                                    newPages = allPages
                                        .slice(scope.$currentPage - pageNumbers, spliceValue(scope.$currentPage))
                        ...
                        function deleteRow(item) {
                            ...
                            var obj = {
                                    params: dparams,
                                    url: buildUrl(null, options.deleteSuffix),
                                    eventType: 'delete'
                                },
                                ajax = {};
                            if (options.formatRequest) {
                        ...
                        function removeRow(item) {
                            var index = -1;
                            if ($pageable.localdata &&
                                $pageable.localdata.length &&
                                (index = $pageable.localdata.indexOf(item)) > -1)
                        ...
                        function saveRow(item, isNew) {
                            ...
                            var rp = routeParams(),
                                ...
                                obj = {
                                    params: { routeParams: rp, data: item },
                                    url: buildUrl(null, options[eventType + 'Suffix']),
                                    eventType: eventType
                                },
                                ajax = {};
                            if (options.formatRequest) {
...
angular.module('ngQuantum.pageable.pagesize', ['ngQuantum.services.templateHelper', 'ngQuantum.services.helpers'])
    .directive('nqPagingSizes',
    [
        '$helpers', '$parse', function($helpers, $parse) {
            return {
                restrict: 'A',
                require: '^nqPageable',
                templateUrl: function(element, attr) {
                    ...
                    if (angular.isDefined(attr.templateUrl))
                        return attr.templateUrl;
                    return attr.nqPagingSizes == 'buttons'
                        ? 'pageable/pagesize.buttongroup.tpl.html'
                        : 'pageable/pagesize.select.tpl.html';
                },
                link: function postLink(scope, element, attr, controller) {
                    var type = attr.nqPagingSizes || 'select';

                    var sizeOptions = angular.isDefined(attr.sizeOptions)
                        ? $helpers.parseConstant(attr.sizeOptions)
                        : controller.$options.sizeOptions;
                    ...
                    scope.$locale.sizesText = ($helpers.parseConstant(attr.sizesText) !== false) &&
                        (attr.sizesText || controller.$options.sizesText || false);
                    angular.forEach(['selectClasses', 'buttonGroupClasses', 'buttonClasses'],
                        function(val, key) {
                            if (angular.isDefined(attr[val]))
                                attr.$observe(val,
                                    function(newVal) {
        ...
        function() {
            return {
                ...
                link: function(scope, element, attr, controller) {
                    ...
                    angular.forEach(['formClasses', 'inputGroupClasses', 'formControlClasses', 'searchButtonClasses'],
                        function(val, key) {
                            attr[val] &&
                                attr.$observe(val,
                                    function(newVal) {
                    ...
                    if (attr.autoSearch && (attr.autoSearch == true) || (attr.autoSearch == 'true')) {
                        scope.$watch('searchTerm',
                            function(newValue, oldValue) {
                                newValue && scope.$searchTable();
                                !newValue && oldValue && scope.$clearSearch();
                            })
                    }
                ...
                link: function(scope, element, attr, controller) {
                    ...
                    if (field) {
                        if (angular.isDefined(attr.useTemplate)) {
                            element.css('position', 'relative')
                            template = angular
                                .element('<span class="column-sorting"><span class="asc-icon"></span><span class="desc-icon"></span></span>');
        ...
        function() {
            return {
                ...
                link: function(scope, element, attr, controller) {
                    ...
                    angular.forEach(['zeroTitle', 'zeroDescription'],
                        function(val, key) {
                            attr[val] &&
                                attr.$observe(val,
                                    function(newVal) {
